...
Short written entries can use `heroId: null` when there are no main-album photos. The story and catalog then use a text cover. Empty albums and routes are omitted along with their navigation links. A trip with no media does not need a source media directory. An archive-only entry keeps its photos separate from the story's date, with their original `year`, optional `memoryEyebrow`, caption and full-frame viewer. A single archival photo is displayed as a large card; the archive cover stays textual when no hero is specified. Missing cover, album, route or region presentation text defaults to an empty string; supplied values are still validated. Optional `peopleNote` adds a note below the participant list.
