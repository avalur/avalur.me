...
(() => {
  ...
  let pending=null;
  function currentAt(time){let current;for(const shot of [...shots,...(data.titleCards||[]).map(card=>({...card,id:card.title,note:''}))].sort((a,b)=>a.start-b.start)){if(time<shot.start||time>=shot.end)continue;if(!current||time>=(shot.start+Math.min(current.end,shot.end))/2)current=shot}return current}
  function update(){
    ...
    for(const button of chapterButtons){if(current&&button.dataset.chapter===photoChapters[current.before_photo||current.id])button.setAttribute('aria-current','true');else button.removeAttribute('aria-current')}
  }
  function seek(time){
  ...
  for(const button of [...photos,...chapterButtons])button.addEventListener('click',()=>{
    ...
    const titleStart=Number(button.dataset.titleStart);seek(button.hasAttribute('data-title-start')&&Number.isFinite(titleStart)?titleStart+0.6:shot.start+Math.min(1.05,(shot.end-shot.start)/2));
